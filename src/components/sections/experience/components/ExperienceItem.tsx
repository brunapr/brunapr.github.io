  ...
  return (
    <div className="flex space-x-4 lg:space-x-6 font-mono w-full">
      <div className="flex flex-col space-y-4 w-10 md:w-8 lg:w-6 items-center justify-start mt-2">
        ...
        <div className="w-[2px] h-full bg-violet-200 dark:bg-slate-600 rounded-full" />
      </div>
      <div className="flex flex-col w-fit">
        ...
        <div className="mt-4 text-slate-800 dark:text-slate-400 space-y-2">
          {
            list.map((item: string, index) => {
              return (
                <div key={"experience_" + index} className="flex space-x-4 text-sm">
                  <span>•</span>
                  <span>{item}</span>
                </div>
              );
